...
''' 
This part is divided as a separete cell for test purposes only, take into 
account only one patient, later on there is description of the remainings patients
'''

test_image_flair=nib.load(TRAIN_DATASET_PATH + 'BraTS20_Training_355/BraTS20_Training_355_flair.nii').get_fdata()
# print("flair.max() ", test_image_flair.max())
#Scalers are applied to 1D so let us reshape and then reshape back to original shape.
# print("flair shape[-1] ", test_image_flair.shape[-1])
# print("flair transform shape ", scaler.fit_transform(test_image_flair.reshape(-1, test_image_flair.shape[-1])).shape)
# print("flair transform back ", scaler.fit_transform(test_image_flair.reshape(-1, 1)).shape)

# test_image_flair=scaler.fit_transform(test_image_flair.reshape(-1, test_image_flair.shapśe[-1])).reshape(test_image_flair.shape)
...
# print(test_image_flair2.shape)
# print(test_image_flair==test_image_flair2)
# test_mx=test_image_flair==test_image_flair2
# print(test_mx.all())
# %%
...
# transformation to the one hot encoding
...
for img in range(len(t2_list)):   #Using t1_list as all lists are of same size
    ...
    temp_image_t2=scaler.fit_transform(temp_image_t2.reshape(-1, 1)).reshape(temp_image_t2.shape)

    temp_image_t1ce=nib.load(t1ce_list[img]).get_fdata()
    # temp_image_t1ce=scaler.fit_transform(temp_image_t1ce.reshape(-1, temp_image_t1ce.shape[-1])).reshape(temp_image_t1ce.shape)
    temp_image_t1ce=scaler.fit_transform(temp_image_t1ce.reshape(-1, 1)).reshape(temp_image_t1ce.shape)
   
    temp_image_flair=nib.load(flair_list[img]).get_fdata()
    # temp_image_flair=scaler.fit_transform(temp_image_flair.reshape(-1, temp_image_flair.shape[-1])).reshape(temp_image_flair.shape)
    temp_image_flair=scaler.fit_transform(temp_image_flair.reshape(-1, 1)).reshape(temp_image_flair.shape)
    ...
    Path("BraTS2020_TrainingData/input_data_3channels/images/").mkdir(parents=True, exist_ok=True)
    Path("BraTS2020_TrainingData/input_data_3channels/masks/").mkdir(parents=True, exist_ok=True)
...
# %%
''' plot all and check for test purposes'''
